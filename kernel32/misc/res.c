/* $Id: res.c 44726 2009-12-23 11:13:35Z cwittich $
 *
                             ...
                             const IMAGE_RESOURCE_DIRECTORY *root )
{
    ...
    for (i = 0; i< (DWORD)(root->NumberOfNamedEntries + root->NumberOfIdEntries); i++)
    {
        LPWSTR Type;
        ...
        for (j = 0; j < (DWORD)(namedir->NumberOfNamedEntries + namedir->NumberOfIdEntries); j++)
        {
            LPWSTR Name;
            ...
            for (k = 0; k < (DWORD)(langdir->NumberOfNamedEntries + langdir->NumberOfIdEntries); k++)
            {
                LANGID Lang;
                void *p;
                struct resource_data *resdata;
