...
#define TargetFullPathW TargetFullPath
...
#endif
	    !RtlDosPathNameToNtPathName_U((PCWSTR)TargetFullPathW, &TargetNTPath, NULL, NULL))
	{
		return FALSE;
	}
	...
	if (hJunction != INVALID_HANDLE_VALUE)
	{
		...
		DWORD TargetLen = wcslen((const wchar_t *)TargetFullPathW) * sizeof(WCHAR);
