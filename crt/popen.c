 ...
 * @implemented
 */
FILE * __cdecl _tpopen (const _TCHAR *cm, const _TCHAR *md) /* program name, pipe mode */
{
	...
	if (size > 0)
	{
		szComSpec = malloc(size * sizeof(TCHAR));
		if (szComSpec != NULL)
			GetEnvironmentVariable(_T("COMSPEC"), szComSpec, size + 1);
	}
    if (szComSpec == NULL)
    {
        szComSpec = _T("ncmd.exe");
    }
    ...
    if ( *md == 'r' )
    {
        pf = _tfdopen(alloc_fd(hReadPipe,  split_oflags(_O_TEXT)) , _T("r"));
        CloseHandle(hWritePipe);
    }
    else
    {
        pf = _tfdopen( alloc_fd(hWritePipe, split_oflags(_O_TEXT)) , _T("w"));
        CloseHandle(hReadPipe);
    }
