 ...
 *		_access (MSVCRT.@)
 */
int __cdecl _access(const char *filename, int mode)
{
 ...
 *		_waccess (MSVCRT.@)
 */
int __cdecl _waccess(const wchar_t *filename, int mode)
{
 ...
 *		_chmod (MSVCRT.@)
 */
int __cdecl _chmod(const char *path, int flags)
{
 ...
 *		_wchmod (MSVCRT.@)
 */
int __cdecl _wchmod(const wchar_t *path, int flags)
{
 ...
 * inheritable.
 */
int __cdecl _dup2(int od, int nd)
{
  int ret;
 ...
 */
int __cdecl _locking(int fd, int mode, LONG nbytes)
{
  BOOL ret;
  DWORD cur_locn;
  HANDLE hand = fdtoh(fd);
 ...
 *		_wfdopen (MSVCRT.@)
 */
FILE* __cdecl _wfdopen(int fd, const wchar_t *mode)
{
...
// *		_futime (MSVCRT.@)
// */
//int __cdecl _futime(int fd, struct _utimbuf *t)
//{
//  HANDLE hand = fdtoh(fd);
//  FILETIME at, wt;
//
 ...
 *		_mktemp (MSVCRT.@)
 */
char * __cdecl _mktemp(char *pattern)
{
  int numX = 0;
  char *retVal = pattern;
  int id;
 ...
 *		_wmktemp (MSVCRT.@)
 */
wchar_t * __cdecl _wmktemp(wchar_t *pattern)
{
  int numX = 0;
 ...
 *              _pipe (MSVCRT.@)
 */
int __cdecl _pipe(int *pfds, unsigned int psize, int textmode)
{
  int ret = -1;
 ...
 *              _sopen (MSVCRT.@)
 */
int __cdecl _sopen( const char *path, int oflags, int shflags, ... )
{
  va_list ap;
  int pmode;
 ...
 *              _wsopen (MSVCRT.@)
 */
int __cdecl _wsopen( const wchar_t* path, int oflags, int shflags, ... )
{
 ...
 *              _open (MSVCRT.@)
 */
int __cdecl _open( const char *path, int flags, ... )
{
  va_list ap;
 ...
 *              _wopen (MSVCRT.@)
 */
int __cdecl _wopen(const wchar_t *path,int flags,...)
{
  const unsigned int len = strlenW(path);
  char *patha = calloc(len + 1,1);
  va_list ap;
 ...
 *		_creat (MSVCRT.@)
 */
int __cdecl _creat(const char *path, int flags)
{
 ...
 *		_wcreat (MSVCRT.@)
 */
int __cdecl _wcreat(const wchar_t *path, int flags)
{
  int usedFlags = (flags & _O_TEXT)| _O_CREAT| _O_WRONLY| _O_TRUNC;
 ...
 */
int __cdecl _open_osfhandle(intptr_t handle, int oflags)
{
  int fd;
 ...
 *		_telli64 (MSVCRT.@)
 */
__int64 __cdecl _telli64(int fd)
{
 ...
 *		_wtempnam (MSVCRT.@)
 */
//wchar_t * __cdecl _wtempnam(const wchar_t *dir, const wchar_t *prefix)
//{
...
// *		_utime (MSVCRT.@)
// */
//int __cdecl _utime(const char* path, struct _utimbuf *t)
//{
...
// *		_wutime (MSVCRT.@)
// */
//int __cdecl _wutime(const wchar_t* path, struct _utimbuf *t)
//{
 ...
 *		_putw (MSVCRT.@)
 */
int __cdecl _putw(int val, FILE* file)
{
  int len;
 ...
 *		fgets (MSVCRT.@)
 */
char * __cdecl fgets(char *s, int size, FILE* file)
{
  int    cc = EOF;
  char * buf_start = s;
 ...
 *              fgetws (MSVCRT.@)
 */
wchar_t * __cdecl fgetws(wchar_t *s, int size, FILE* file)
{
  int    cc = WEOF;
  wchar_t * buf_start = s;
 ...
 *		fwrite (MSVCRT.@)
 */
size_t __cdecl fwrite(const void *ptr, size_t size, size_t nmemb, FILE* file)
{
 ...
 *		_fputwchar (MSVCRT.@)
 */
wint_t __cdecl _fputwchar(wint_t wc)
{
 ...
 *		_fsopen (MSVCRT.@)
 */
FILE * __cdecl _fsopen(const char *path, const char *mode, int share)
{
  FILE* file;
 ...
 *		_wfsopen (MSVCRT.@)
 */
FILE * __cdecl _wfsopen(const wchar_t *path, const wchar_t *mode, int share)
{
...
int __cdecl _flsbuf(int c, FILE* file);

/*********************************************************************
 *		fputc (MSVCRT.@)
 */
int __cdecl fputc(int c, FILE* file)
{
  if(file->_cnt>0) {
    *file->_ptr++=(char)c;
    file->_cnt--;
 ...
 *		fread (MSVCRT.@)
 */
size_t __cdecl fread(void *ptr, size_t size, size_t nmemb, FILE* file)
 ...
 *
 */
FILE* __cdecl freopen(const char *path, const char *mode,FILE* file)
{
  int open_flags, stream_flags, fd;
 ...
 *
 */
FILE* __cdecl _wfreopen(const wchar_t *path, const wchar_t *mode,FILE* file)
{
  int open_flags, stream_flags, fd;
 ...
 *		fputs (MSVCRT.@)
 */
int __cdecl fputs(const char *s, FILE* file)
{
 ...
 *		fputws (MSVCRT.@)
 */
int __cdecl fputws(const wchar_t *s, FILE* file)
{
 ...
 *		putc (MSVCRT.@)
 */
int __cdecl putc(int c, FILE* file)
{
  return fputc(c, file);
}
 ...
 *		_wrename (MSVCRT.@)
 */
int __cdecl _wrename(const wchar_t *oldpath,const wchar_t *newpath)
{
  TRACE(":from %S to %S\n",oldpath,newpath);
 ...
 *		setbuf (MSVCRT.@)
 */
void __cdecl setbuf(FILE* file, char *buf)
{
 ...
 *		ungetc (MSVCRT.@)
 */
int __cdecl ungetc(int c, FILE * file)
{
	if (c == EOF)
		return EOF;
	if(file->_bufsiz == 0 && !(file->_flag & _IONBF)) {
 ...
 *              ungetwc (MSVCRT.@)
 */
wint_t __cdecl ungetwc(wint_t wc, FILE * file)
{
	wchar_t mwc = wc;
	char * pp = (char *)&mwc;
	int i;
 ...
 */
int __cdecl _setmaxstdio(int newmax)
{
    int res;
    if( newmax > 2048)
      res = -1;
